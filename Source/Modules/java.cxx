...
public:
    ...
    for (int i = 1; i < argc; i++) {
      if (argv[i]) {
        if (strcmp(argv[i], "-package") == 0) {
          ...
          Delete(public_string);
          public_string = NewStringEmpty();
          Delete(protected_string);
          protected_string = NewString("internal");
  ...
  virtual int top(Node *n) {
    ...
    // Generate the intermediary class
    {
      ...
      if (imclass_package && package)
        packageStr = NewStringf("package %s.%s", package, imclass_package);
      else if (imclass_package)
        packageStr = NewStringf("package %s", imclass_package);
      else if (package)
        packageStr = NewStringf("package %s", package);
    ...
    if (!ImportMode) {
      ...
      if ((enum_feature != SimpleEnum) && symname && typemap_lookup_type) {
        ...
        if (proxy_flag && is_wrapping_class()) {
          ...
          if (kotlin_use)
            kotlinReindent(enum_code, enum_feature == SimpleEnum);
          Printv(f_enum,
...
#pragma GCC diagnostic pop
  ...
  virtual int constantWrapper(Node *n) {
    ...
    if (value) {
      ...
      if (kotlin_use) {
        String *kvalueFetch = kotlinConstExpr(cvalue);
        String *kvalue = kvalueFetch ? kvalueFetch : cvalue;
        SwigType *valueType = Getattr(n, "valuetype");
        String *return_type_convert = NULL;
        if (valueType) {
          String *valTypeStr = SwigType_str(valueType, 0);
          String *constTypeStr = SwigType_str(t, 0);
          /* Check if we need to convert */
          if (Cmp(valTypeStr, constTypeStr)) {
            /*
             * We use 'jstype' typemap as it is the source of the Kotlin constant type.
             * 'from<C value type>' attribute store the Kotlin proper convertion.
             */
            String *name = NewStringf("tmap:jstype:from%s", valTypeStr);
            return_type_convert = Getattr(n, name);
            Delete(name);
          }
          Delete(valTypeStr);
          Delete(constTypeStr);
        }
        if (return_type_convert) {
          Printf(constants_code, "%s\n", return_type_convert);
          Replaceall(constants_code, "$input", kvalue);
        } else {
          Printf(constants_code, "%s\n", kvalue);
        }
        Delete(kvalueFetch);
  ...
  static void kotlinReindent(String *&s, bool clean_semicolon = true) {
    if (!s || Len(s) == 0)
      return;
    // remove end of line semicolon
    if (clean_semicolon)
      Replaceall(s, ";\n", "\n");
  ...
  static String *kotlinParsePrimary(const char *&pp) {
    kotlinSkipSpace(pp);
    const char *p = pp;
    char c = *p;
    if (c == '(') {
      String *inner = kotlinParseExpr(++pp);
      if (!inner)
        return NULL;
      kotlinSkipSpace(pp);
      if (*pp != ')') {
  ...
  // Convert C++ string to Kotlin
  static String *kotlinStr(const char *&pp) {
    String *r = NewString("\"");
    pp++;
    for (;;) {
      const char c = *pp++;
      ...
      case '\\':
        ...
        if (isodigit(*pp)) {
          ...
          // All other escape sequance are paased as is
          Putc('\\', r);
          Putc(*pp++, r);
        }
        break;
  ...
  static String *kotlinChar(const char *&pp) {
    ...
    if (*pp == '\\') {
      ...
      if (isodigit(pp[1])) {
        // Convert number to kotlin
        int n = pp[1] - '0';
        int e = 2;
        while (e < 4 && isodigit(pp[e]))
          // octal value
          n = n * 8 + pp[e++] - '0';
        if (pp[e] == '\'') {
  ...
  static String *kotlinParseUnary(const char *&pp) {
    kotlinSkipSpace(pp);
    String *o = NULL, *r = NULL;
    switch (*pp) {
    case '~':
      o = kotlinParseUnary(++pp);
      if (!o)
        return NULL;
      r = NewStringf("(%s).inv()", o);
      Delete(o);
      return r;
    case '-':
      o = kotlinParseUnary(++pp);
  ...
  static String *kotlinParseExpr(const char *&pp, int min_prec = 0) {
  ...
  static String *kotlinConstExpr(const String *cexpr) {
    if (!cexpr)
      return NULL;
    const char *p = Char(cexpr);
    String *out = kotlinParseExpr(p);
    if (!out)
      return NULL;
    kotlinSkipSpace(p);
    if (*p != 0) {
